...
if (cmnThm.length > 0) {
  ...
  // Initialize theme based on localStorage or set default to 'light'
  $(document).ready(function () {
    let theme = localStorage.getItem("theme");
    
    // Set default theme if none is stored
    if (!theme) {
      theme = "light";  // Default theme if no theme is set in localStorage
      localStorage.setItem("theme", theme);  // Save the default theme in localStorage
    }
