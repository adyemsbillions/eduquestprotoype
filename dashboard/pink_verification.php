...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['receipt'])) {
    ...
    if (in_array($file_type, $allowed_types) && $_FILES['receipt']['error'] == 0) {
        ...
        if (move_uploaded_file($_FILES['receipt']['tmp_name'], $file_path)) {
            $sql = "INSERT INTO verification_receipts (user_id, receipt_path, status, verification_type) VALUES (?, ?, 'pending', 'pink')";
            $stmt = $conn->prepare($sql);
...
// Check if the user has an approved receipt for pink verification
$sql_check = "SELECT status FROM verification_receipts WHERE user_id = ? AND verification_type = 'pink' AND status = 'approved'";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['request_verification']) && $has_approved_receipt) {
    $sql_update = "UPDATE users SET pink_verified = 1 WHERE id = ?";
    $stmt_update = $conn->prepare($sql_update);
    $stmt_update->bind_param("i", $user_id);
    if ($stmt_update->execute()) {
        $request_message = "<p class='success'>Pink Verification requested successfully!</p>";
    } else {
...
<head>
    ...
    <style>
        ...
        .submit-btn {
            background-color: #f06292; /* Pink color for button */
            color: white;
            border: none;
            padding: 12px 20px;
            font-size: 16px;
...
<body>
    <div class="container">
        <h1 class="header">Pink Verification</h1>
        ...
        // Check eligibility
        if ($verified != 1) {
            echo "<p class='error'>You must be verified to get a pink verification.</p>";
        } elseif ($gender !== 'female') {
            echo "<p class='error'>Only female users are eligible for pink verification.</p>";
        } else {
            // Display upload form
            ...
            if ($has_approved_receipt) {
                ...
                echo "<input type='submit' name='request_verification' value='Request Pink Verification' class='submit-btn'>";
                echo "</form>";
