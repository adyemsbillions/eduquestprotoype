...
<head>
  ...
  <style>
    ...
    .comments-box.active { transform: translateY(0); }
    .upload-form { 
      position: fixed;
      top: 0;
      left: 0;
...
<body class="bg-black text-white">
  ...
  <div id="upload-form" class="upload-form">
    <div class="relative w-full h-full flex items-center justify-center p-4">
      <button id="close-upload" class="absolute top-4 right-4 text-2xl text-gray-300 hover:text-white transition-colors">
      ...
      <form action="" method="post" enctype="multipart/form-data" class="w-full max-w-md space-y-4">
        ...
        <button type="submit" class="w-full p-3 bg-gradient-to-r from-primary to-primary-dark rounded-lg hover:from-primary-dark hover:to-primary transition-colors font-medium">
          Upload Reel
        </button>
      </form>
    </div>
  ...
  <div class="reel-feed" id="reel-feed">
    <?php foreach ($reels as $reel): ?>
      <div class="reel-item" id="reel-<?php echo $reel['id']; ?>">
        ...
        <!-- Overlay Content -->
        <div class="absolute bottom-0 left-0 w-full p-4 pb-20 bg-gradient-to-t from-black/80 to-transparent pointer-events-none">
          <!-- Reel Header -->
          <div class="reel-header flex items-center gap-3 mb-3">
            <a href="view_profile.php?user_id=<?php echo htmlspecialchars($reel['user_id']); ?>" class="flex-shrink-0">
              <img src="<?php echo htmlspecialchars($reel['profile_picture']); ?>" 
                   alt="Profile" 
          ...
          <p class="text-sm font-medium text-white line-clamp-2"><?php echo htmlspecialchars($reel['description']); ?></p>
        </div>
        ...
        <div class="absolute right-2 bottom-24 flex flex-col gap-2 items-center">
          ...
          <button class="upload-toggle relative">
            <div class="w-10 h-10 flex items-center justify-center rounded-full bg-gradient-to-r from-primary to-primary-dark backdrop-blur-sm">
              <i class="fas fa-plus text-white"></i>
            </div>
          </button>
          
          <!-- Like Button -->
          <form method="POST" class="flex flex-col items-center like-form" data-reel-id="<?php echo $reel['id']; ?>">
            ...
            <button type="submit" class="relative">
              <div class="w-10 h-10 flex items-center justify-center rounded-full bg-black/30 backdrop-blur-sm">
                <i class="<?php echo $reel['user_liked'] ? 'fas fa-heart text-red-500' : 'far fa-heart text-white'; ?> text-2xl"></i>
              </div>
              <span class="text-xs font-semibold text-white mt-1 like-count"><?php echo formatNumber($reel['likes']); ?></span>
            </button>
          </form>
          ...
          <button class="toggle-comments relative" data-reel-id="<?php echo $reel['id']; ?>">
            ...
            <span class="text-xs font-semibold text-white mt-1"><?php echo formatNumber($reel['comments']); ?></span>
          </button>
          
          <!-- Share Button -->
          <form method="POST" class="flex flex-col items-center share-form" data-reel-id="<?php echo $reel['id']; ?>">
            ...
            <button type="submit" class="relative">
              ...
              <span class="text-xs font-semibold text-white mt-1 share-count"><?php echo formatNumber($reel['shares']); ?></span>
            </button>
          </form>
          
          <!-- Related Reels Link -->
          <a href="related.php?reel_id=<?php echo $reel['id']; ?>" class="w-10 h-10 flex items-center justify-center rounded-full bg-black/30 backdrop-blur-sm">
        ...
        <div class="comments-box" data-reel-id="<?php echo $reel['id']; ?>">
          <div class="h-full flex flex-col">
            ...
            <div class="px-4 py-3 border-b border-gray-700 flex items-center justify-between">
              <h3 class="text-lg font-bold text-white"><?php echo formatNumber($reel['comments']); ?> comments</h3>
              ...
              <?php else: ?>
                <?php foreach ($reel['comments_array'] as $comment): ?>
                  <div class="comment flex gap-3 py-3">
                    ...
                    <div class="flex-1 min-w-0">
                      <div class="flex items-center gap-1">
                        <strong class="text-sm font-semibold text-white">@<?php echo htmlspecialchars($comment['username']); ?></strong>
                      ...
                      <p class="text-sm text-gray-100"><?php echo htmlspecialchars($comment['comment_text']); ?></p>
            ...
            <!-- Comment Input -->
            <form method="POST" class="comment-form px-4 py-3 border-t border-gray-700 flex gap-2" data-reel-id="<?php echo $reel['id']; ?>">
              ...
              <div class="flex-1 relative">
                <textarea name="comment_text" placeholder="Add a comment..." 
                          ...
                          rows="1"></textarea>
                <button type="submit" class="absolute right-2 bottom-2 text-primary hover:text-primary-dark transition-colors">
      ...
      doubleTapAreas.forEach(area => {
        ...
        area.addEventListener('click', (e) => {
          ...
          if (tapLength < 300 && tapLength > 0) {
            ...
            if (!likeButton.classList.contains('fas') || !likeButton.classList.contains('text-red-500')) {
              $(form).trigger('submit');
            }
        ...
        $.ajax({
          ...
          success: function(response) {
            if (response.success) {
              ...
              if (response.liked) {
                likeButton.removeClass('far fa-heart text-white').addClass('fas fa-heart text-red-500');
              } else {
                likeButton.removeClass('fas fa-heart text-red-500').addClass('far fa-heart text-white');
              }
        ...
        $.ajax({
          ...
          success: function(response) {
            if (response.success) {
              ...
              const currentCount = parseInt(commentCountEl.text().replace(/[^0-9]/g, '')) || 0;
